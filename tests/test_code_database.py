...
def test_setup_db(mocker):
    ...
    mock_create_engine = mocker.patch(
        'code_management.code_database.create_engine')
    mock_sessionmaker = mocker.patch(
        'code_management.code_database.sessionmaker')
...
def test_code_storage():
    ...
    added_class = session.query(CodeClass).filter_by(
    ...
    added_function = session.query(CodeFunction).filter_by(
        function_name=function_name).first()
